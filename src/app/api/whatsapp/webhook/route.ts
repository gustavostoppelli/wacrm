    ...
    for (const change of entry.changes) {
      ...
      for (let i = 0; i < value.messages.length; i++) {
        ...
        await processMessage(
          ...
          // Which channel this message arrived on (migration 037) —
          // stamped onto the conversation so a contact who talks to
          // two channels of this account gets two separate threads.
          config.id
        )
      }
    }
...
/**
 * Parse a Meta-shaped inbound message + fire the shared,
 * provider-agnostic pipeline (contact/conversation find-or-create,
 * persistence, Flows/automations/AI/webhook dispatch — see
...
async function processMessage(
  ...
  accessToken: string,
  // Which whatsapp_config row this message arrived on (migration 037).
  channelId: string
) {
  ...
  // `mediaType` is intentionally unused beyond building the proxy URL
  // inside parseMessageContent — the schema has no media_type column.
  void mediaType

  // Stickers are images under the hood (MessageBubble renders <img>);
  // any other unmapped type falls back to 'text' — both handled by
  // inbound-message.ts's own ALLOWED_CONTENT_TYPES safety net, except
  // 'sticker' which isn't a valid ContentType at all and must be
  // remapped here before it reaches the shared pipeline.
  const contentType = message.type === 'sticker' ? 'image' : message.type
  ...
  await processInboundMessage({
    ...
    contentText: contentText ?? (contentType === 'text' ? message.text?.body ?? null : null),
    mediaUrl,
    interactiveReplyId,
...
// Contact/conversation find-or-create, persistence, and every downstream
