...
export async function sendMessageToConversation(
  ...
  // Resolve which channel this conversation sends through (migration
  // 037). Falls back to the account's sole channel for conversations
  // that predate multi-channel support — identical to the old
  // "the account's one config" behaviour for every single-channel
  // account.
  const channel = await resolveChannelForConversation(db, accountId, conversationId);
  if (!channel) {
    throw new SendMessageError(
